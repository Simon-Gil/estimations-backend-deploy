...
class ProposalService {
    ...
    /**
     * Obtiene las propuestas asociadas a una oportunidad.
     * @param opportunity - Oportunidad de la cual obtener las propuestas.
     * @returns Lista de propuestas asociadas a la oportunidad.
     */
    async getProposalsByOpportunity(opportunity) {
        const proposals = opportunity.proposals;
        const proposalsDTO = (0, class_transformer_1.plainToInstance)(proposal_dto_1.ProposalDTO, proposals);
        return proposalsDTO;
    }
    /**
    ...
    async getProposals(user) {
        ...
        if (user) {
            ...
            proposals = await proposal_repository_1.proposalRepo.findRelatedToUser(user);
        }
        else {
            proposals = await proposal_repository_1.proposalRepo.find({ relations: ['estimation'] });
        }

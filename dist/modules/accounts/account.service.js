...
class AccountService {
    ...
    async getAllAccounts(user) {
        let accounts = await account_repository_1.accountRepo.find({
            relations: ['priceConfig', 'technicalManager', 'commercialManager']
        });
    ...
    */
    async createAccount(email, name, legalName, cif, tlf, commercialManagerId, technicalManagerId, isCustomer) {
        ...
        if (!commercialManagerId) {
            throw new AppError_1.AppError('No se ha recibido responsable comercial', 400);
        }
        else {
            const commercialManager = await user_repository_1.userRepo.findOne({
                where: { id: commercialManagerId }
            });
            if (commercialManager) {
                newAccount.commercialManager = commercialManager;
            }
        }
        // Asignacion de responsable tecnico
        if (technicalManagerId) {
            const technicalManager = await user_repository_1.userRepo.findOne({
                where: { id: technicalManagerId }
            });
            if (technicalManager) {
                newAccount.technicalManager = technicalManager;
            }
        }
        // Asignacion de isCustomer
        if (isCustomer !== undefined) {
            newAccount.isCustomer = isCustomer;
        }
        // Asignacion de PriceConfig
    ...
    async getAccountOpportunities(accountId, user) {
        let account = await exports.accountService.getAccountById(accountId, ['opportunities', 'opportunities.proposals', 'opportunities.typology']);
     ...
     *
     * La función permite actualizar varios atributos de una cuenta comercial, tales como el nombre, el correo electrónico,
     * el responsable comercial y técnico, así como el campo booleano `isCustomer`. Además, verifica que el correo electrónico
     * no esté siendo utilizado por otra cuenta.
     *
    ...
    async updateAccount(id, name, email, legalName, cif, tlf, commercialManagerId, technicalManagerId, isCustomer) {
        ...
        if (commercialManagerId) {
            ...
            accountEntity.commercialManager = commercialManager;
        }
        // Asignación de responsable técnico
        if (technicalManagerId) {
            const technicalManager = await user_service_1.userService.getUserById(technicalManagerId);
            accountEntity.technicalManager = technicalManager;
        }
        // Asignación de campo booleano isCustomer
        if (isCustomer !== undefined) {
            accountEntity.isCustomer = isCustomer;
        }

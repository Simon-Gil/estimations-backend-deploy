...
class OpportunityService {
     ...
     * @param commercialManagerId - (Opcional) El identificador del responsable comercial de la oportunidad.
     * @param technicalManagerId - (Opcional) El identificador del responsable técnico de la oportunidad.
     *
     ...
     */
    async createOpportunity(accountId, name, requirements, typologyId, commercialManagerId, technicalManagerId) {
        // Obtener cuenta
        ...
        // Obtener responsable comercial si se ha introducido, de lo contrario asignar el de la cuenta
        let commercialManager;
        if (commercialManagerId) {
            commercialManager = await user_service_1.userService.getUserById(commercialManagerId);
        }
        else {
            commercialManager = account.commercialManager;
        }
        // Obtener responsable técnico si se ha introducido, si no se asigna el de la cuenta, si existe
        let technicalManager;
        if (technicalManagerId) {
            technicalManager = await user_service_1.userService.getUserById(technicalManagerId);
        }
        else {
            if (account.technicalManager) {
                technicalManager = account.technicalManager;
            }
        }
     ...
     * - Responsable comercial de la oportunidad.
     * - Responsable técnico de la oportunidad.
     *
     ...
     * @param technicalManagerId - (Opcional) El identificador del nuevo responsable técnico de la oportunidad.
     * @param commercialManagerId - (Opcional) El identificador del nuevo responsable comercial de la oportunidad.
     *
     ...
     */
    async updateOpportunity(id, name, requirements, technicalManagerId, commercialManagerId) {
    ...
    * junto con sus estimaciones. Luego, utiliza proposalService para obtener las propuestas detalladas de la oportunidad.
    *
    ...
    */
    async getOpportunityProposals(opportunityId) {
        ...
        const proposals = await proposal_service_1.proposalService.getProposalsByOpportunity(opportunity);
        return proposals;
    }
    /**
     ...
     * de la cuenta asociada a la oportunidad debe ser actualizada a "true".
     *
     ...
     */
    async updateOpportunityStatus(opportunityId, status, user) {
        const opportunityEntity = await this.getById(opportunityId);
    ...
    async getOpportunities(user) {
        let opportunities = await opportunity_repository_1.opportunityRepo.find({
            relations: ['commercialManager', 'technicalManager', 'proposals', 'typology']
        });
        // Cargamos permisos

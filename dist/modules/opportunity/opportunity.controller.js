...
class OpportunityController {
    async updateOpportunity(req, res, next) {
        try {
            ...
            const data = req.body;
            const updatedOpportunity = await opportunity_service_1.opportunityService.updateOpportunity(id, data.name, data.requirements, data.technicalManager, data.commercialManager);
    ...
    async updateOpportunityStatus(req, res, next) {
        try {
            ...
            const id = req.params.id;
            const opportunityUpdated = await opportunity_service_1.opportunityService.updateOpportunityStatus(id, status, req.user);
    ...
    async getOpportunities(req, res, next) {
        try {
            const opportunities = await opportunity_service_1.opportunityService.getOpportunities(req.user);

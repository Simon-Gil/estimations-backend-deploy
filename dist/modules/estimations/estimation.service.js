...
const final_estimation_dto_1 = require("./dtos/final-estimation.dto");
...
class EstimationService {
    ...
    async createEstimation(proposalId) {
        ...
        return (0, class_transformer_1.plainToInstance)(final_estimation_dto_1.FinalEstimationDTO, savedEstimation);
    }
    /**
    ...
    async completeEstimation(estimationId) {
        ...
        if (unfinishedTasks) {
            throw new AppError_1.AppError('No se puede finalizar la estimación mientras haya tareas pendientes', 404);
        }
    ...
    /**
     * Obtiene una estimación específica con los costes calculados y desglosados.
     * Este método utiliza una utilidad para procesar los datos de la estimación y calcular
     * los costes basados en los perfiles de precios asociados.
     *
     * @param estimationId - Identificador único de la estimación que se desea calcular.
     * @returns Una promesa que resuelve con un objeto `FinalEstimationDTO` que contiene los datos de la estimación
     * y los costes calculados de manera desglosada.
     * @throws `AppError` si la estimación no se encuentra en el sistema.
     */
    async getCalculatedEstimation(estimationId) {
        ...
        const profilePrices = estimation.opportunity.account.priceConfig.profilePrices;
        // Llamar a la función de cálculo
        const calculatedEstimation = await estimation_utility_1.estimationUtility.processEstimation(estimation, profilePrices, true);
        return calculatedEstimation;
    }
    /**
     * Obtiene una lista de estimaciones que han sido finalizadas.
     *
     * @returns Una promesa que resuelve con un array de objetos `EstimationDTO` que representan
     * las estimaciones con estado "DONE".
     */
    async getDoneEstimations() {
        const estimations = await estimation_repository_1.estimationRepo.find({
            where: { status: EstimationStatus_1.EstimationStatus.DONE }
        });
     ...
     */
    async getEstimationDetail(estimationId) {
        const estimation = await this.getById(estimationId, ['tasks', 'tasks.hrsTaskProfiles', 'estimationUsers.user', 'tasks.hrsTaskProfiles.profile',
            'tasks.secondLevelCategory.firstLevelCategory', 'proposal', 'proposal.opportunity.commercialManager', 'proposal.opportunity.technicalManager']);
        const structuredEstimation = await estimation_utility_1.estimationUtility.processEstimation(estimation, false);
        return (0, class_transformer_1.plainToInstance)(final_estimation_dto_1.FinalEstimationDTO, structuredEstimation);
    }
    /**
    ...
    async getEstimationTasks(estimationId, user) {
        const estimation = await this.getById(estimationId, ['tasks, estimationUsers']);

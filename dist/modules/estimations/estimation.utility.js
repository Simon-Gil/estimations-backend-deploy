...
const first_level_category_repository_1 = require("../company-structure/task-categories/first-level-category.repository");
...
const first_level_category_dto_1 = require("../company-structure/task-categories/first-level-category.dto");
const second_level_category_dto_1 = require("../company-structure/task-categories/second-level-category.dto");
...
class EstimationUtility {
     ...
     */
    async processEstimation(estimation, profilePricesOrCalculated, calculated) {
        const finalEstimation = (0, class_transformer_1.plainToInstance)(final_estimation_dto_1.FinalEstimationDTO, estimation);
        finalEstimation.minCost = 0;
        finalEstimation.maxCost = 0;
        ...
        // Precios por perfil y flag de cálculo
        const profilePrices = Array.isArray(profilePricesOrCalculated) ? profilePricesOrCalculated : [];
        const shouldCalculate = calculated ?? (typeof profilePricesOrCalculated === 'boolean' ? profilePricesOrCalculated : true);
        ...
        finalEstimation.firstLevelCategories = Array.from(firstLevelCategoriesMap.values());
        finalEstimation.hMin = 0;
        finalEstimation.hMax = 0;
        finalEstimation.hMin = Array.from(firstLevelCategoriesMap.values()).reduce((acc, category) => acc + (category.hMin || 0), 0);
        finalEstimation.hMax = Array.from(firstLevelCategoriesMap.values()).reduce((acc, category) => acc + (category.hMax || 0), 0);
        if (shouldCalculate) {
            finalEstimation.minCost = Array.from(firstLevelCategoriesMap.values()).reduce((acc, category) => acc + category.minCost, 0);
            finalEstimation.maxCost = Array.from(firstLevelCategoriesMap.values()).reduce((acc, category) => acc + category.maxCost, 0);
        }

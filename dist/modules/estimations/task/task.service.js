...
const second_level_category_service_1 = require("../../company-structure/task-categories/second-level-category.service");
...
class TaskService {
     ...
     * @throws {AppError} Si ya existe una tarea con la misma descripción en la estimación.
     */
    ...
    async updateTaskStatus(status, taskId) {
        const task = await this.getById(taskId, ['hrsTaskProfiles']);
        ...
        if (pendingHrsTaskProfiles && status === "done") {
            throw new AppError_1.AppError('No se puede finalizar una tarea con perfiles estimables pendientes', 400);
        }

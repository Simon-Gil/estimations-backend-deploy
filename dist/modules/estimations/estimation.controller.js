...
class EstimationController {
    ...
    async getDoneEstimations(req, res, next) {
        try {
            const doneEstimations = await estimation_service_1.estimationService.getDoneEstimations();
            res.status(200).json(doneEstimations);
        }
        catch (err) {
            next(err);
        }
    ...
    async getEstimationDetail(req, res, next) {
        try {
            ...
            if (await (0, authorization_utility_1.checkAuthorization)(user, 'readPriceConfig', 'account')) {
                estimationDetail = await estimation_service_1.estimationService.getCalculatedEstimation(id);
            }
            else {
                estimationDetail = await estimation_service_1.estimationService.getEstimationDetail(id);
                estimationDetail.minCost = undefined;
                estimationDetail.maxCost = undefined;
            }
